...
class MainCls(wx.Frame):
    def __init__(self, parent, id):
        ...
        self.nm_button = wx.CheckBox(self.Panel, -1, 'q in data is in [1/nm]? default unit is [1/aa]', size = (350, -1))
...
#       "DisableSANSFnc()",
    ...
    def cappFnc(self, event):
        ...
        if capp_version == "capp_windows.exe":
            ...
            print("p(r) succesfully calculated - see folder with pdb file")
    ...
    def CalcPqFnc(self, event):
        ...
        if os.path.isfile(pr_filename):
            ...
            print("Please calculate the p(r) for this PDB with these options before you calculate P(q)")
            Message = 'Please calculate the p(r) for this PDB with these options before you calculate P(q)'
            wx.MessageBox(Message, "CaPP", wx.OK | wx.ICON_INFORMATION)
        ...
        if capp_version == "capp_windows.exe":
            ...
            print("P(q) succesfully calculated - see folder with pdb file")
...
### Define function for Fitting Bg and scale
    def FitFnc(self, event):
        ...
        if self.DataPathStr == 'Non':
            ...
            if os.path.isfile(Pq_filename):
                file = open(self.DataPathStr,'r')
            else:
                print("Please calculate the P(q) with these options before you fit")
                Message = 'Please calculate the P(q) with these options before you fit'
                wx.MessageBox(Message, "CaPP", wx.OK | wx.ICON_INFORMATION)
