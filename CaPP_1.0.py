...
#       "DisableSANSFnc()",
    ...
    def RgFnc(self, event):
        ...
        if capp_version == "capp_windows.exe":
            Output = "%s %s %s %s %s %s %s %s %s -g %s" % (capp_version, XN_choice, solvent, WL_choice, WL_contrast, Exclude_WL_choice, Bilayer_thickness, Resolution_choice, Resolution, self.PDBPathStr)
        else:
            Output = "./%s %s %s %s %s %s %s %s %s -g %s" % (capp_version, XN_choice, solvent, WL_choice, WL_contrast, Exclude_WL_choice, Bilayer_thickness, Resolution_choice, Resolution, self.PDBPathStr)
