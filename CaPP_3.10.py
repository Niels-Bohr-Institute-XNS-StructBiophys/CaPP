...
class MainCls(wx.Frame):
    def __init__(self, parent, id):
        ...
        self.left_SANS_d = wx.StaticText(self.Panel, -1, "Perdeut., chain A,B,...,F =")
        ...
        self.Perdeut_text = wx.StaticText(self.Panel, -1, 'Enter -1 for non-existing chains, else a number between 0 and 100 %', size = (410, -1))
...
#       "EnableSANSFnc()",
    ...
    def cappFnc(self, event):
        ...
        if self.fitPDB2_button.GetValue():
            Output = "%s/%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s" % (programpath, capp_version, XN_choice, solvent, Perdeut_choice_A, perdeut_A, Perdeut_choice_B, perdeut_B, Perdeut_choice_C, perdeut_C, Perdeut_choice_D, perdeut_D, Perdeut_choice_E, perdeut_E, Perdeut_choice_F, perdeut_F, X_choice, PrcSucrose, WL_choice, WL_contrast, Exclude_WL_choice, Bilayer_thickness, Resolution_choice, Resolution, self.PDB2PathStr)
        else:
        ...
        if self.SANS_button.GetValue():
            ...
            if value > 0.0:
                string_value_B = "_%1.0f" % value
            value = float(self.SANS_perdeut_box_C.GetValue())
            if value > 0.0:
                string_value_C = "_%1.0f" % value
            value = float(self.SANS_perdeut_box_D.GetValue())
            if value > 0.0:
                string_value_D = "_%1.0f" % value
            value = float(self.SANS_perdeut_box_E.GetValue())
            if value > 0.0:
                string_value_E = "_%1.0f" % value
            value = float(self.SANS_perdeut_box_F.GetValue())
            if value > 0.0:
                string_value_F = "_%1.0f" % value
            filename = filename + "_N" + string_value1 + "_P" + string_value_A + string_value_B  + string_value_C  + string_value_D + string_value_E + string_value_F
            filename_nowater = short_filename + "_N" + string_value1 + "_P" + string_value_A + string_value_B + string_value_C + string_value_D + string_value_E + string_value_F              
        else:
        ...
        if self.fitPDB2_button.GetValue():
            ...
            if self.SANS_button.GetValue():
                ...
                if value > 0.0:
                    string_value_F = "_%1.0f" % value
                filename2 = filename2 + "_N" + string_value1 + "_P" + string_value_A + string_value_B  + string_value_C + string_value_D + string_value_E + string_value_F
                filename2_nowater = short_filename2 + "_N" + string_value1 + "_P" + string_value_A + string_value_B + string_value_C + string_value_D + string_value_E + string_value_F              
            else:
                value = float(self.SAXS_solvent_box.GetValue())
                string_value = "%1.0f" % value
    ...
    def PlotprFnc(self,filename, PDBID):
        ...
        if capp_version == "capp_windows.exe":
            ...
            if self.SANS_button.GetValue():
                structure_name = structure_name + ", SANS " + self.SANS_solvent_box.GetValue() + "% D2O, [" + self.SANS_perdeut_box_A.GetValue() + "," + self.SANS_perdeut_box_B.GetValue() + "," + self.SANS_perdeut_box_C.GetValue() + "," + self.SANS_perdeut_box_D.GetValue() + "," + self.SANS_perdeut_box_E.GetValue() + "," + self.SANS_perdeut_box_F.GetValue() + "] % Deut."
            else:
    ...
    def FitFnc(self, event):
        ...
        # define Pq filename
            ...
            if self.SANS_button.GetValue():
                ...
                if value > 0.0:
                    string_value_F = "_%1.0f" % value
                filename = filename + "_N" + string_value1 + "_P" + string_value_A + string_value_B + string_value_C + string_value_D + string_value_E + string_value_E
            else:
                value = float(self.SAXS_solvent_box.GetValue());
                string_value = "%1.0f" % value
                filename = filename + "_X" + string_value
        ...
        # define Pq filename 2
            if self.fitPDB2_button.GetValue():
                ...
                if self.SANS_button.GetValue():
                    ...
                    value = float(self.SANS_perdeut_box_B.GetValue())
                    if value > 0.0:
                        string_value_B = "%_1.0f" % value
                    value = float(self.SANS_perdeut_box_C.GetValue())
                    if value > 0.0:
                        string_value_C = "%_1.0f" % value
                    value = float(self.SANS_perdeut_box_D.GetValue())
                    if value > 0.0:
                        string_value_D = "%_1.0f" % value
                    value = float(self.SANS_perdeut_box_E.GetValue())
                    if value > 0.0:
                        string_value_E = "%_1.0f" % value
                    value = float(self.SANS_perdeut_box_F.GetValue())
                    if value > 0.0:
                        string_value_F = "%_1.0f" % value
                    filename2 = filename2 + "_N" + string_value1 + "_P" + string_value_A + string_value_B + string_value_C + string_value_D + string_value_E + string_value_F
                else:
                    value = float(self.SAXS_solvent_box.GetValue())
                    string_value = "%1.0f" % value
